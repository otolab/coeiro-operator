...
技術用語とキャラクター名が事前定義されています。詳細は `src/core/dictionary/default-dictionaries.ts` を参照してください。
...
1. **永続性**: COEIROINK APIへの登録はプロセス再起動でリセットされます（COEIRO Operatorは起動時に自動再登録します）
