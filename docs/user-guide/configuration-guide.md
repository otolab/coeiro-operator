...
### 設定の優先順位
1. **内蔵デフォルト設定** (基盤): 公式キャラクターの内蔵設定（speakerIdで識別）
2. **動的音声検出** (自動): COEIROINKサーバーから音声情報取得（speakerIdでマッチング）
3. **ユーザー設定** (カスタマイズ): `~/.coeiro-operator/config.json`
...
| **audio** | | | | 音声処理設定 |
| | `defaultRate` | Number | - | デフォルト話速（WPM）。未指定時は話者固有速度 |
| | `latencyMode` | String | `"balanced"` | レイテンシモード: `"ultra-low"`, `"balanced"`, `"quality"` |
| | `splitMode` | String | `"punctuation"` | テキスト分割モード: `"none"`, `"punctuation"`, `"small"`, `"medium"`, `"large"` |
...
| **tools** | | | | MCPツールグループの有効/無効 |
| | `operator` | Boolean | `true` | operator_assign, operator_release, operator_status, operator_available, operator_styles |
| | `speech` | Boolean | `true` | say |
| | `playback` | Boolean | `true` | queue_status, queue_clear, playback_stop, wait_for_task_completion |
| | `dictionary` | Boolean | `true` | dictionary_register |
| | `debug` | Boolean | `true` | debug_logs |

未指定のグループはデフォルトで有効です。`tools`セクション自体を省略した場合は全ツールが有効になります。不要なツールを無効化することで、MCPクライアントのツール一覧を簡素化しトークン消費を削減できます。
...
| ameno | アメノちゃん | cc31327e-6617-446f-ba41-eb88f89b8fce |
...
3. **キャラクターのデフォルト**
   - Character定義の`defaultStyle`（`config.json`でカスタマイズ可能）
