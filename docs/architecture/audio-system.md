...
```
COEIROINK API → WAV → PCM → リサンプリング → ローパスフィルター → ノイズリダクション → @echogarden/audio-io 出力
...
### 音声出力ライブラリ（@echogarden/audio-io）

**技術**: @echogarden/audio-io `createAudioOutput()`
- **対応プラットフォーム**: Windows / macOS / Linux
- **特徴**: コールバックベースAPI、低レイテンシ・高品質出力
