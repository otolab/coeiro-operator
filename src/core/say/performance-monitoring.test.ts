...
describe('音声合成システム パフォーマンス監視', () => {
  ...
  describe('メモリリーク検知テスト', () => {
    // Issue #50: Node.js GC仕様によりメモリ測定が不安定 - より精密な測定手法が必要
    test.skip('連続音声合成でメモリリークが発生しないこと', async () => {
      // より精密なメモリ測定手法
      if (global.gc) {
        global.gc(); // 初期GC
    ...
    // Issue #50: Node.js GC仕様によりメモリ測定が不安定 - より精密な測定手法が必要
    test.skip('大量の並行リクエスト処理後にメモリが適切に解放されること', async () => {
  ...
  describe('リソース効率性テスト', () => {
    // Issue #50: Node.js GC仕様によりメモリ測定が不安定 - より精密な測定手法が必要
    test.skip('長文処理において適切なチャンクサイズでメモリ効率が保たれること', async () => {
      // 長文テキスト（約1000文字）
