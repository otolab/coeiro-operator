...
describe('MCP Server Tools Tests', () => {
  describe('Tool Callback Logic Tests', () => {
    ...
    test('operator_assign ツールロジック - エラーケース', async () => {
      ...
      // エラーケースのテスト
      await expect(processOperatorAssign({ operator: 'invalid' })).rejects.toThrow(
        'オペレータ割り当てエラー: オペレータが見つかりません'
      );
    });
    ...
    test('parallel_generation_control ツールロジック - enable', async () => {
      ...
      // 無効なアクションのテスト
      await expect(processParallelControl({ action: 'invalid' })).rejects.toThrow(
        '並行生成制御エラー: 無効なアクション: invalid'
      );
    });
    ...
    test('debug_logs ツールロジック', async () => {
      ...
      // 無効なアクションのテスト
      await expect(processDebugLogs({ action: 'invalid' })).rejects.toThrow(
        'ログ取得エラー: 無効なアクション: invalid'
      );
    });
    ...
    test('operator_styles ツールロジック', async () => {
      ...
      const processOperatorStyles = async (args: any) => {
        ...
        Object.entries(characterInfo.available_styles).forEach(
          ([styleId, style]: [string, any]) => {
      ...
      // エラーケースのテスト
      await expect(processOperatorStyles({ character: 'invalid' })).rejects.toThrow(
        "スタイル情報取得エラー: キャラクター 'invalid' が見つかりません"
      );
    });
  });
  ...
  describe('Tool Response Format Validation', () => {
    ...
    test('引数バリデーション処理の検証', () => {
      ...
      // エラーケース
      expect(() => validateArgs({}, ['message'])).toThrow(
        '必須フィールドが不足しています: message'
      );
      expect(() => validateArgs({ rate: 200 }, ['message', 'voice'])).toThrow(
        '必須フィールドが不足しています: message, voice'
      );
      expect(() => validateArgs(null, ['message'])).toThrow(
        '必須フィールドが不足しています: message'
      );
    });
  });
