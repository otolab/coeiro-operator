      ...
      const checkResponse = () => {
        ...
        if (this.output.length > 0) {
          ...
          this.output.slice(-5).forEach((line, i) => {
            console.log(
              `  [${this.output.length - 5 + i}] ${line.substring(0, 100)}${line.length > 100 ? '...' : ''}`
            );
          });
        }
        ...
        if (this.errorOutput.length > 0) {
          ...
          this.errorOutput.slice(-5).forEach((line, i) => {
            console.log(
              `  [${this.errorOutput.length - 5 + i}] ${line.substring(0, 100)}${line.length > 100 ? '...' : ''}`
            );
          });
        }
...
describe('Punctuation Mode Tests', () => {
  ...
  const defaultConfigPath = path.resolve(
    __dirname,
    '../../../test-configs/default-split-mode-config.json'
  );
  ...
  describe('Punctuation Mode Functionality', () => {
    test('MCPツールでsplitMode設定を確認', async () => {
      ...
      const debugResponse = await testRunner.sendJsonRpcRequest(
        'tools/call',
        {
          name: 'debug_logs',
          arguments: {
    ...
    test('句読点分割モードでの音声合成（chunkMode未指定）', async () => {
      ...
      const sayResponse = await testRunner.sendJsonRpcRequest(
        'tools/call',
        {
          name: 'say',
          arguments: {
  ...
  describe('Default SplitMode Behavior', () => {
    ...
    test('punctuationモードの実際の分割動作を直接確認', async () => {
      ...
      // オペレータを割り当て（音声合成に必要）
      await testRunner.sendJsonRpcRequest(
        'tools/call',
        {
      ...
      // 句読点を含む複数文のテキストで音声合成
      const testText =
        'これは最初の文です。次に二番目の文があります！最後に三番目の文で終わります。';
      ...
      if (configurationWorking) {
        console.log(
          '✅ Configuration working correctly: splitMode undefined defaults to punctuation mode'
        );
      } else {
    ...
    test('明示的にpunctuationを指定した場合と同等の動作', async () => {
      ...
      const initResponse1 = await testRunner.sendJsonRpcRequest(
        'initialize',
        {
          protocolVersion: '2024-11-05',
          capabilities: { tools: {} },
      ...
      const initResponse2 = await testRunner.sendJsonRpcRequest(
        'initialize',
        {
          protocolVersion: '2024-11-05',
          capabilities: { tools: {} },
  ...
  describe('Fallback Behavior', () => {
    test('設定ファイルがない場合のフォールバック動作', async () => {
      ...
      // フォールバック設定での初期化ログを確認
      const fallbackLogs = output.stderr.filter(
        line => line.includes('fallback') || line.includes('Failed to initialize')
      );
