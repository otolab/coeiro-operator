  ...
  describe('Echo Back MCP Server', () => {
    test('サーバーの起動と初期化', async () => {
      ...
      expect(testRunner['output'].stdout).toEqual(
        expect.arrayContaining([expect.stringContaining('Echo MCP Server ready')])
      );
    }, 10000);
  ...
  describe('出力チャネル分離', () => {
    test('各出力チャネルの正常な分離', async () => {
      ...
      expect(testRunner['output'].errorMessages).toEqual(
        expect.arrayContaining([expect.stringContaining('Test error: test-error')])
      );
    }, 10000);
  });
    ...
    test('連続JSON-RPCリクエストの処理', async () => {
      ...
      const requests = [
        { jsonrpc: '2.0', method: 'tools/list', id: 10 },
        {
          jsonrpc: '2.0',
          method: 'tools/call',
          params: { name: 'echo', arguments: { message: 'test1' } },
          id: 11,
        },
        {
          jsonrpc: '2.0',
          method: 'tools/call',
          params: { name: 'echo', arguments: { message: 'test2' } },
          id: 12,
        },
      ];
