...
describe('MCPServiceE2ETester', () => {
  ...
  describe('基本的なライフサイクル', () => {
    ...
    it('未起動時のツール呼び出しはエラーになる', async () => {
      ...
      await expect(tester.callTool('operator_status')).rejects.toThrow('Tester is not started');
    });
  });
  ...
  describe('待機機能', () => {
    ...
    it('特定の状態になるまで待機できる', async () => {
      // すでにREADY状態なので、すぐに完了するはず
      await expect(tester.waitForState(MCPServerState.READY, 1000)).resolves.toBeUndefined();
    });

    it('タイムアウトでエラーになる', async () => {
      // UNINITIALIZED状態にはならないので、タイムアウトになるはず
      await expect(tester.waitForState(MCPServerState.UNINITIALIZED, 100)).rejects.toThrow(
