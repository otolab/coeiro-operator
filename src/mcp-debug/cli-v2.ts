...
class MCPDebugCLIv2 {
  ...
  private showHelp(): void {
    console.log(`
MCP Debug CLI v2 - Improved MCP Server Debugging Tool

Usage: mcp-debug-v2 [options] <target-server-file> [-- <child-options>...]
...
Key Improvements:
  ...
  ✅ State management for server lifecycle
  ✅ Clean separation of concerns
  ✅ No arbitrary timeouts - waits for actual protocol events

Examples:
  # Basic usage (non-interactive)
  echo '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"operator_status","arguments":{}},"id":1}' | \\
    mcp-debug-v2 dist/mcp/server.js
  
  # Interactive mode
  mcp-debug-v2 --interactive dist/mcp/server.js
  
  # Debug mode with child options
  mcp-debug-v2 --debug dist/mcp/server.js -- --config custom.json
    `);
  }
...
async function main(): Promise<void> {
  ...
  const cli = new MCPDebugCLIv2(options);
...
export { MCPDebugCLIv2, type CLIOptions };